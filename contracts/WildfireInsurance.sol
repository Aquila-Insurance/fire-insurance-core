...
contract WildfireInsurance is ChainlinkClient, Ownable {
    ...
    uint256 private CONTRACT_NETWORK = 4;
    address public oracle_address = address(0);
    string public oracle_jobid = "";
    ...
    mapping(bytes32 => policy) private claims;
    mapping(address => mapping(address => policy)) private policies;
    ...
    constructor() public Ownable() {
        ...
        oracle_address = 0x8Ef5fF73BE22FFa44F1Ed36ccAeFef86e47e0C64;
        oracle_jobid = "6d670bf298f6475e8b5ab5b891e6b1b6";
    }
    ...
    function buyPolicy(
        address payable _policyAddress,
        uint256 _productId,
        string memory _iplCoordinates
    ) public payable {
        require(msg.value > 0, "No value policy purchase");
        require(
            msg.value >= products[_productId].price,
            "Insufficient tx value for policy purchase"
        );

        uint256 payableAmount = msg.value *
            products[_productId].payoutMultiplier;
        liquidityPool += uint256(msg.value);

        policy memory ipl = policy(
            msg.sender,
            _policyAddress,
            _productId,
            _iplCoordinates,
    ...
    function viewPoliciesIdx() public view returns (address[] memory) {
        address[] memory adr = policyIndex[msg.sender];
        return adr;
    }

    function tryClaim(address _policyAddress) public {
        policy memory ipl = policies[msg.sender][_policyAddress];

        if (ipl.masterAddress != address(0)) {
            //Insurance Found - Proceed to check if policy is triggered
            Chainlink.Request memory req = buildChainlinkRequest(
                stringToBytes32(oracle_jobid),
                address(this),
                this.fulfillClaimInquiry.selector
            );
            req.add("spec", "firecontrol_spec");
            req.add("id1", ipl.iplCoordinates);
            claims[req.id] = ipl;
            sendChainlinkRequestTo(oracle_address, req, ORACLE_PAYMENT);
        }
    }

    function fulfillClaimInquiry(bytes32 _requestId, bytes32 _data)
        public
    ...
    function getLPBalance() public view onlyOwner returns (uint256) {
